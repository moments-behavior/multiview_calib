...
from multiview_calib.intrinsics import (
    enforce_monotonic_distortion,
...
def process_image(
    ...
    ret, corners = cv2.findChessboardCorners(
        gray,
        (inner_corners_height, inner_corners_width),
    ...
    if debug:
        gray = cv2.drawChessboardCorners(
            gray, (inner_corners_height, inner_corners_width), imgp, ret
        )
        imageio.imsave(
            os.path.join(debug_folder, os.path.basename(filename_image)), gray
        )
...
def main(
    ...
    print(
        "intrinsic_guess:", intrinsic_guess if len(intrinsic_guess) else False
    )
    ...
    if load_keypoints:
        keypoints = utils.json_read(
            os.path.join(output_folder, "keypoints.json")
        )
        ...
        objp = np.zeros(
            (inner_corners_height * inner_corners_width, 3), np.float32
        )
        objp[:, :2] = np.mgrid[
            0:inner_corners_height, 0:inner_corners_width
        ].T.reshape(-1, 2)
        objp[:, :2] *= square_sizes
        ...
        objpoints = [
            objp.copy() for r in res if r is not None
        ]  # 3d point in real world space
        imgpoints = [
            r.copy() for r in res if r is not None
    ...
    plt.savefig(
        os.path.join(output_folder, "detected_keypoints.jpg"),
        bbox_inches="tight",
    )
    ...
    def reprojection_error(mtx, distCoeffs, rvecs, tvecs):
        ...
        for i in range(len(objpoints)):
            ...
            reproj_error += cv2.norm(
                imgpoints[i], imgpoints2, cv2.NORM_L2
            ) / len(imgpoints2)
    ...
    grid_norm, is_monotonic = probe_monotonicity(
        mtx, distCoeffs, newcameramtx, image_shape, N=100, M=100
    )
    ...
    plt.grid()
    plt.savefig(
        os.path.join(output_folder, "monotonicity.jpg"), bbox_inches="tight"
    )

    proj_undist_norm = np.vstack(
        [
            cv2.projectPoints(
                objpoints[i], rvecs[i], tvecs[i], np.eye(3), None
            )[0].reshape(-1, 2)
    ...
    if force_monotonicity:
        is_monotonic = is_distortion_function_monotonic(
            distCoeffs, range=(0, monotonic_range, 1000)
        )
        if is_monotonic:
            print(
                "The distortion function is monotonic in the range (0,{:0.2f})".format(
                    monotonic_range
                )
            )
        else:
            print(
                "The distortion function is not monotonic in the range (0,{:0.2f})".format(
                    monotonic_range
                )
            )
        ...
        if not is_monotonic:
            print(
                "Trying to enforce monotonicity in the range (0,{:.2f})".format(
                    monotonic_range
                )
            )
            ...
            for objp, imgp in zip(objpoints, imgpoints):
                _, rvec, tvec = cv2.solvePnP(
                    objp, imgp, mtx, distCoeffs
                )  # is this the best?
                rvecs_new.append(rvec)
                tvecs_new.append(tvec)

            reproj_error = reprojection_error(
                mtx, distCoeffs, rvecs_new, tvecs_new
            )
            print(
                "mono: RMS Reprojection Error: {}, Total Reprojection Error: {}".format(
                    ret, reproj_error
                )
            )
    ...
    for i, filenames_image in enumerate(filename_images):
        ...
        try:
            ...
            dst = cv2.circle(
                dst, (int(mtx[0, 2]), int(mtx[1, 2])), 6, (255, 0, 0), -1
            )

            imageio.imsave(
                os.path.join(
                    undistorted_folder, os.path.basename(filenames_image)
                ),
                dst,
            )
        except:
            print(
                "Something went wrong while undistorting the images. The distortion coefficients are probably not good. You need to take a new set of calibration images."
            )
            # sys.exit(0)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_folder", "-o", type=str, default="./output", required=False
    )
    parser.add_argument(
        "--description",
        "-d",
    ...
    parser.add_argument(
        "--fix_aspect_ratio", "-far", action="store_true", required=False
    )
    parser.add_argument(
        "--zero_tangent_dist", "-ztg", action="store_true", required=False
    )
    parser.add_argument(
        "--criteria_eps",
        "-eps",
    ...
    parser.add_argument(
        "--save_keypoints", action="store_true", required=False
    )
    parser.add_argument(
        "--load_keypoints", action="store_true", required=False
    )
